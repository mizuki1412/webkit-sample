...
export function gridDemo(change){
  return {
    top: (change?.top === null || change?.top === undefined) ? 30 : change.top,
    left: (change?.left === null || change?.left === undefined) ? 15 : change.left,
    right: (change?.right === null || change?.right === undefined) ? 20 : change.right,
    bottom: (change?.bottom === null || change?.bottom === undefined) ? 5 : change.bottom,
  }
}
...
export function chartConfig(option, config) {
  ...
  if(config.darkTheme){
    ...
    option = Object.assign(option, themeObj)
  }
  return option
}

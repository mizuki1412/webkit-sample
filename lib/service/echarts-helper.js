...
// 柱状点击事件
export function triggerOnGridClick(vChartRef, func = function (index) {}){
  ...
  vChartRef.chart.getZr().on('click', function(param) {
    ...
    if (vChartRef.chart.containPixel('grid',pointInPixel)) {
      ...
      let xIndex=vChartRef.chart.convertFromPixel({seriesIndex:0},[param.offsetX, param.offsetY])[0];
      // let yIndex=myChart.convertFromPixel({seriesIndex:0},[param.offsetX, param.offsetY])[1];
      func(xIndex)
    }
  });
}
...
export function antRenderDate(dtKey){
  return (text, record)=> {
    let v = getObjValByKeys(record, dtKey)
    return record?formatDate(v):''
  }
}
...
export function antRenderDateTime(dtKey){
  return (text, record)=> {
    let v = getObjValByKeys(record, dtKey)
    return record?formatDateTime(v):''
  }
}
...
export function antRenderDateWeek(dtKey){
  return (text, record)=> {
    let v = getObjValByKeys(record, dtKey)
    let dj = dayjsObj(v)
    return dj?dj.format("YY-MM-DD 周dd"):''
  }
}

// 序号
export function antRenderIndex(){
  return (text, record, index)=> index+1
}

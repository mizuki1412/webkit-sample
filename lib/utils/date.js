...
export function formatMilliseconds(ms, zh, formatterStr = "hh:mm:ss") {
  ...
  if (zh) {
    ...
    DD && (ret += String(day).padStart(2,'0'))
    DD && hh && (ret += ":")
    hh && (ret += String(hour).padStart(2,'0'))
    hh && mm && (ret += ":")
    mm && (ret += String(min).padStart(2,'0'))
    mm && ss && (ret += ":")
    ss && (ret += String(sec).padStart(2,'0'))
    ss && (mmm || m) && (ret += ".")
    mmm && (ret += String(ms).padStart(3,'0'))
    m && (ret += String(ms).padStart(1,'0'))
  }
  return ret
}
